...
| Sensitive endpoint abuse | Attackers could brute-force profile provisioning/profile reads or flood payment, refund, and webhook endpoints. | Sensitive endpoints are protected by a selectable rate-limit backend. Local/test use bounded in-memory counters by trusted-proxy-aware client identity. Production startup rejects blank or in-memory backends and requires Redis shared counters or gateway/WAF enforcement. The filter still returns sanitized `429` Problem Details when application counting rejects a request. | `AbuseRateLimitFilter`, `RateLimitBackendConfiguration`, `InMemoryRateLimitBackend`, `RedisRateLimitBackend`, `GatewayRateLimitBackend`, `RequestIdFilter`, `AbuseRateLimitFilterTest`, `DeploymentProfileGuardTest`, `README.md`. |
...
| Stock stream connection abuse | A client could hold many long-lived SSE connections and exhaust memory or servlet resources. | Stock streams require the `stock:stream` scope, default to a five-minute timeout, and cap concurrent subscribers per product and client. The client key uses authenticated subject when available and trusted request IP otherwise. Over-limit subscriptions return sanitized `429` Problem Details. | `StockEventStreamController`, `StockEventBroadcaster`, `StockEventStreamControllerTest`, `StockEventBroadcasterTest`. |
...
- When `RATE_LIMIT_BACKEND=gateway`, enforcement depends on external gateway/WAF policy outside the application and must be deployed with trusted proxy configuration. When `RATE_LIMIT_BACKEND=redis`, Redis availability becomes part of the production abuse-control path.
