...
| Audit tampering | An attacker or operator could modify historical audit records without detection. | Audit events store canonical payload hashes and previous hashes. Verification recalculates the chain and reports the first broken event. Database triggers reject direct updates and deletes on `audit_event`, and migration fails if unhashed legacy audit rows remain. Real regulated production should still anchor audit evidence outside the primary database, such as WORM storage or an externally keyed signature chain. | `AuditHashService`, `AuditHashVerificationService`, Flyway `V13__add_audit_hash_chain.sql`, Flyway `V24__make_audit_event_append_only.sql`, `AuditHashVerificationTest`, `AuditEventController`. |
...
- Customer profiles are provisioned from authenticated OAuth2 subjects; anonymous local password registration is not exposed.
- Database append-only guarantees for ledger and audit records are demonstrated by migrations/tests, but production hardening should also restrict application database permissions and anchor audit evidence outside the primary database.
