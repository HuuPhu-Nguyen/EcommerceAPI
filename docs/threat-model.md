...
| Sensitive endpoint abuse | Attackers could brute-force profile provisioning/profile reads or flood payment, refund, and webhook endpoints. | An in-memory limiter throttles sensitive endpoints by remote address and returns sanitized `429` Problem Details. Production multi-instance deployments should replace or augment this with Redis, gateway, or WAF rate limiting. | `AbuseRateLimitFilter`, `AbuseRateLimitFilterTest`, `README.md`. |
